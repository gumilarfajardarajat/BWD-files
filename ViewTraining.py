...
class ViewTraining:
    def __init__(self,root):
        ...
        txColor = "#FFFFFF"
        self.tr_addroot = ""
        self.tr_path = padi.index_tr()[0]['img']
        self.tr_id = padi.index_tr()[0]['id']
        self.tr_idx = 0
        ...
        self.tr_frameinfo = Label(root, bg=bgColor, text="Data Training", anchor="w",fg=txColor ,font=("Whiteney 20 bold")).grid(row=0, column=0,pady=20, sticky='WENS',padx=20)
        self.tr_btn_tr_add = Button(root, bg="#7289DA", text="Tambah" ,font=("Whiteney 10 bold") , width=10, fg=txColor,relief=FLAT, command= lambda: self.showTrainAdd(root)).grid(row=0, column=1)
        ...
        self.tr_lmean = Label(root, bg=bgColor, text="Mean", anchor="w",font=("Whiteney 12 bold") , fg=txColor).grid(row=1, column=0, sticky='WENS',padx=20)
        ...
        self.tr_lmean = Label(root, bg=bgColor, text="Standar Deviasi", anchor="w",font=("Whiteney 12 bold"), fg=txColor).grid(row=1, column=1, sticky='WENS',padx=20)
        ...
        self.tr_llevel = Label(root, bg=bgColor, text="Level :",anchor="w",font=("Whiteney 12 bold"), fg=txColor).grid(row=5, column=0, sticky='WENS',padx=20)
        ...
        self.tr_lwaktu = Label(root, bg=bgColor, text="Waktu :",anchor="w",font=("Whiteney 12 bold"), fg=txColor).grid(row=6, column=0, sticky='WENS',padx=20)
        ...
        self.tr_btn_change = Button(root, bg="#FFFFFF", text="Ganti Gambar",relief=FLAT,font=("Whiteney 10 bold"),width=18, command=lambda: self.uploadTrain()).grid(row=5, column=2)

        self.tr_btn_del = Button(root, bg="#FFFFFF", text="Hapus",font=("Whiteney 10 bold"),width=10,relief=FLAT, command=lambda : self.modalDelete(root)).grid(row=7, column=0, pady=10)
        self.tr_btn_save = Button(root, bg="#FFFFFF", text="Simpan",font=("Whiteney 10 bold"),width=10,relief=FLAT, command=lambda : self.tr_save()).grid(row=7, column=1)
        self.tr_btn_detail = Button(root, bg="#FFFFFF", text="Detail",font=("Whiteney 10 bold"),width=10,relief=FLAT, command=lambda: self.showTrainDetail(self.tr_path)).grid(row=7, column=2)

        # self.tr_lchoose = Label(root, text="Data Training",font=("Whiteney 18 bold"),bg=bgColor,fg=txColor).grid(row=8, column=0, columnspan=3, sticky='WENS',padx=20)
        ...
        self.tr_listbox.grid(row=9, column=0, columnspan=3, rowspan=3, sticky='WENS', padx=20)
    ...
    def tr_delete(self,root,rtrain):
        id = self.tr_id
        result = PadiController().destroy(id)
        self.tr_listbox.delete(self.tr_idx)
        ViewNavigation(root).labeljmltr['text'] = len(PadiController().index_tr())
        rtrain.destroy()
    ...
    def tr_add(self,root):
        ...
        self.tr_listbox.insert(len(Padi().all_tr()), [(len(Padi().all_tr())), result.name,id])
    ...
    def modalDelete(self,root):
        bgColor = "#202225"
        txColor = "#FFFFFF"
        id = self.tr_id
        rtrain = Toplevel()
        label1 = Label(rtrain, bg=bgColor, text="Perhatian :",anchor="w",font=("Whiteney 12 bold"), fg=txColor).grid(row=0,column=0, columnspan=2)
        label1 = Label(rtrain, bg=bgColor, text="Apakah anda yakin ingin menghapus data ini ?", anchor="w", font=("Whiteney 12 bold"), fg=txColor).grid(row=1, column=0, columnspan=2)
        btntidak = Button(rtrain, bg="#FFFFFF", text="Batal", font=("Whiteney 10 bold"), width=10, relief=FLAT, command=lambda : rtrain.destroy()).grid(row=2, column=0)
        btnya = Button(rtrain, bg="red",fg="#FFFFFF", text="Hapus",font=("Whiteney 10 bold"),width=10,relief=FLAT, command=lambda : self.tr_delete(root,rtrain)).grid(row=2, column=1)
        rtrain.configure(bg=bgColor)
        rtrain.mainloop()
